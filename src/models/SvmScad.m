function [w, b] = SvmScad(x, y, errorPenaltyConstant, lambda, alpha)
...
objL(n+2+2*m:n+1+3*m) = errorPenaltyConstant * -1 * ones(m,1) * (c4 + c3); %q
objL(n+2+3*m:n+1+4*m) = errorPenaltyConstant * ones(m,1) * c3; %p

objQ = zeros(n + 1 + 5*m);
objQ(1:n,1:n) = eye(n); %w
objQ(n+2:n+1+m,n+2+m:n+1+2*m) = errorPenaltyConstant * lambda * eye(m); %xi z
objQ(n+2+2*m:n+1+3*m, n+2+4*m: n+1+5*m) = errorPenaltyConstant * -1 * c1 * eye(m); %q u
objQ(n+2+2*m:n+1+3*m, n+2:n+1+m) = errorPenaltyConstant * c2 * eye(m); %q xi
objQ(n+2+3*m:n+1+4*m, n+2+4*m: n+1+5*m) = errorPenaltyConstant * c1 * eye(m); %p u
objQ(n+2+3*m:n+1+4*m, n+2:n+1+m) = errorPenaltyConstant * -1 * c2 * eye(m); %p xi
